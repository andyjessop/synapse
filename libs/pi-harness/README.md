...
**Synapse tool events:** When `PiReviewRequest.emitHarnessEvent` is set (the `review-pr` reactor does this), the SDK loads the **`synapse-pi-dev`** Pi extension (`libs/pi-harness/src/extensions/synapse-pi-dev-extension.ts`), which emits durable **`pi.tool-call.started.v1`** / **`pi.tool-call.completed.v1`** events for each Pi tool invocation. **`started`** carries bounded **`args.summary`** (what is being read/grepped/written/bash’d). **`completed`** repeats that summary plus optional **`result_summary`** (line counts, bash exit code, ok/failed — never full file bodies). Monotonic **`timeline_order`** keeps dev run artifacts and live CLI lines ordered when `createdAt` ties.
