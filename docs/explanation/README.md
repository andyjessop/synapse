---
title: Explanation
kind: explanation
owner: docs
status: current
updated: 2026-05-20
freshness_triggers:
  - docs/**
---

# Explanation

Explanation pages build understanding of concepts and trade-offs.

## Start here

- [Architecture overview](architecture-overview.md)
- [Event runtime](event-runtime.md)
- [Runtime manifest](runtime-manifest.md) — why manifests replaced worker registration files

## All explanation pages

- [architecture-overview.md](architecture-overview.md)
- [event-runtime.md](event-runtime.md)
- [agents-and-adapters.md](agents-and-adapters.md)
- [runtime-manifest.md](runtime-manifest.md)
- [observability-model.md](observability-model.md)
- [local-first-development.md](local-first-development.md)
